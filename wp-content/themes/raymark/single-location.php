...
<div id="primary" class="content-area">
    ...
    function intro() {
        ...
        $fields = array( 'address', 'phone', 'website', 'email', 'map_marker' );
        ...
        $address = nl2br( $location['address'] );
        if( ! empty( $address ) ) {
            $address = sprintf( '<div class="address">%s%s</div>', get_theme_icon( 'address' ), $address );
        }
        $phone = _s_format_telephone_url( $location['phone'] ), $location['phone'] );
